...
Double_t fPu241e[6];
 ...
 ifstream file("fissionphuber.txt");
   ...
   file>>isono;
   cout<<isono<<endl;
   for(Int_t i=0;i<6;i++)file>>fno>>fU235[i]>>fU235e[i];
   file>>isono;
   cout<<isono<<endl;
   for(Int_t i=0;i<6;i++)file>>fno>>fPu239[i]>>fPu239e[i];
   file>>isono;
   cout<<isono<<endl;
   for(Int_t i=0;i<6;i++)file>>fno>>fPu241[i]>>fPu241e[i];
  
   file>>isono;
   for(Int_t i=0;i<3;i++)file>>fno>>fU238[i]>>fU238e[i];
}

/*
TVector3 GetVertexPostion (void)
{
 Double_t xv,yv,zv,flag;
 TVector2 tpos; 
 TVector3 vpos,vpos1;
...
Double_t CalNeutrinoSpectrum(Double_t E_nu, Double_t cos_theta)
{ 
  //Double_t E_nu=x[0];
  //Double_t cos_theta=x[1];
 ...
 /*
 const Int_t NDimention =7;
 //const Double_t fU235[]={2.70156, -5.90054, 4.67757, -1.11063, -0.0789809, 0.0660638, 0.00211228,
 //                      -0.00177409, 0.000104067, 4.10544, -2.22289, 0.0437163};
 const Double_t fU235[]={3.1882, -0.70654, -0.336322, 0.110402, 0.00307667, -0.00373764, 0.000309321, 
                       5.30531, -1.41786};//pol7+exp2(Emin=1.5,Emid=6,Emax=8)
 const Double_t fU238[]={4.13676, -1.6741, 0.127912, 0.0297204, -0.00194553, -0.000846528, 8.67637e-05, 
                       4.78605, -1.26563};//pol7+exp2(Emin=1.5,Emid=6,Emax=8)
 const Double_t fPu239[]={3.37978, -1.48562, 0.12359, 0.0270622, -0.00185714, -0.000777481, 7.96712e-05, 
                       5.64435, -1.55669};//pol7+exp2(Emin=1.5,Emid=6,Emax=8)
 const Double_t fPu241[]={3.05987, -0.579083, -0.333624, 0.0967984, 0.0037107, -0.00323844, 0.000255672, 
 ...
 */
 ...
 /*
 if(E_nu<=EMID){
 for(Int_t nd=0;nd<NDimention;nd++){
 ...
 */
 //if(E_nu>1.8&&E_nu<10){
   for(Int_t nd=0;nd<6;nd++){
     rate235+=fU235[nd]*pow(E_nu,nd);
     rate239+=fPu239[nd]*pow(E_nu,nd);
     rate241+=fPu241[nd]*pow(E_nu,nd);
   }
   for(Int_t nd=0;nd<3;nd++)rate238+=fU238[nd]*pow(E_nu,nd);
   //}

 Double_t totalRate=exp(rate235)*FRateU235+exp(rate238)*FRateU238+exp(rate239)*FRatePu239+exp(rate241)*FRatePu241;
...
Double_t GetNeutrinoMomentum(TVector3 nudirect, Double_t *posimom,Double_t *neutronmom)
{
  ...
  Double_t y,tmpy,Ee,tmpEe,En,cos_alpha,alpha;
  Int_t maxeve=(Int_t)((EMAX-EMIN)/dE);

  tmpy=1.0e+08;
  tmpEe=-10000.;

  for(Int_t i=0;i<maxeve;i++){
    Ee=i*dE;
...
void NeutrinoGenTotal_PHuber(string st, Int_t NEVENT)
{
  stringstream ss;

 Double_t Xv,Yv,Zv,Px,Py,Pz; 
 //Double_t NuDirect[3]={-1.,0.,0.};
 Double_t positronmom[3],neutronmom[3],E_nu;
...
Int_t main(Int_t argc, char** argv) {
  ...
  return root_Neutrino_generetor();
}
#endif
